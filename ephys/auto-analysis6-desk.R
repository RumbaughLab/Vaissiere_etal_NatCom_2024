...
# copy files ####
dir.create('Y:/Vaissiere/00-ephys temp/MATLA/Nov/28')
file.copy(from=list.files('C:/Users/invivo-ephys/Documents/MATLA/2017/Nov/28', full.names = TRUE), 
          to='Y:/Vaissiere/00-ephys temp/MATLA/Nov/28', 
          overwrite = TRUE) 
...
# data light
obtainDataFromGroup(51, baselineTrace = 200) 
dataPeak(51, 5010, 5060); peak.resp

# graph light
for (i in c(51)){graphSummaryTPS(i)}
...
sID.list<-as.character(unique(exp.d[c(271:length(exp.d$sID)),'sID']))
sID.listExcluded<-c('','123','766','767')
...
fwrite(temp, 'temp.csv'); temp<-fread('temp.csv')
...
ggplot(subset(peak.resp, !sID %in% toplottv2 & !desired.mWmm2 %in% toplottv), 
       aes(x=desired.mWmm2, y=minPeak, group=geno, color=geno)) +
        ...
        ylab("LFP (mW)")+
        theme.tv
quartz.save('pilotdata_supp3.pdf', type = 'pdf')
dev.size()
...
ggplot(subset(b, desired.mWmm2 == 10), aes(x=timeMs, y=mean, color=geno))+
        facet_wrap(~depth)+
        geom_ribbon(aes(ymax=upr,ymin=lwr, fill = geno), color=NA, alpha =.2)+
        geom_line(size=1)
...
ggplot(subset(temp, timeMs>=display.begin*10 & timeMs<=display.end*10 & desired.mWmm2==10), 
       aes(y=value, x=timeMs, color=geno))+ #subset(temp, id>=0 & id<=4000)
        #geom_point()+
        facet_wrap( ~depth)+
        geom_line(aes(group=sID), alpha = 0.6)+
        theme.tv
        geom_smooth(method='loess', aes(fill=geno))
